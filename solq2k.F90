...
!        direction x3
!
      subroutine solq2k(ind)
      use param
      use tridiag
      use local_arrays, only : rhs, q2, forclo
...
!$OMP PRIVATE(kc,ic,ackl_b)
        ...
        do ic=1,n1m
          do kc=1,n3m
            ...
            if (kc .eq. 1) then
              rhst(kc,ic,jc)=rhst(kc,ic,jc)*ackl_b + dqb2s(ic,jc)*ugkks
            elseif (kc .eq. n3m) then
              rhst(kc,ic,jc)=rhst(kc,ic,jc)*ackl_b + dqb2n(ic,jc)*ugkkn
            else
