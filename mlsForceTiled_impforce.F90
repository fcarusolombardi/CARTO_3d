...
!     --------------------------------------------------
      ...
      real(DP), dimension(3) :: P1(3),P2(3),P3(3)
...
!     --------------------------------------------------------
      ...
      do inp=1,Nparticle
      do ntr=fstart(inp),fend(inp)
        if(pind(6,ntr).ge.kstart-1.and.pind(6,ntr).le.kend-1)then
            dum_for(ntr)=0

!     collision for tiles
            ci = pind(1,ntr); cj=pind(2,ntr); ck=pind(3,ntr)
        
           if(inp.ge.3.and.inp.le.7)then
                 if(coll(ci,cj,ck).eq.0)   coll(ci,cj,ck)=inp
                 if(coll(ci,cj,ck).ne.inp) coll(ci,cj,ck)=-1
           end if
...
!     initialise pre-factor matrix
      ...
      pind_i(1)=pindt(1,tile)-1;pind_o(1)=pindt(1,tile)+1
      pind_i(2)=pindt(2,tile)-1;pind_o(2)=pindt(2,tile)+1
      pind_i(3)=pindt(3,tile)-1;pind_o(3)=pindt(3,tile)+1
...
!     For moving particles
      ...
      Forx = vel_tri(1,ntr)-Um1
      Fory = vel_tri(2,ntr)-Um2
      Forz = vel_tri(3,ntr)-Um3
...
!     of grid spacing
      ...
      do k=pind_i(3),pind_o(3)
       do j=pind_i(2),pind_o(2)
        ...
        mvol_t=mvol_t+ptxAB(1,inw)*Hboxx(k)
      ...
      do k=pind_i(3),pind_o(3)
       do j=pind_i(2),pind_o(2)
        do i=pind_i(1),pind_o(1)
            ...
            for_xc(i,j,k)=ptxAB(1,inw)*cel_fx
            for_yc(i,j,k)=ptxAB(1,inw)*cel_fy
            for_zc(i,j,k)=ptxAB(1,inw)*cel_fz
            !Frcs(1,inw) =   ptxAB(1,inw)*cel_fx
            !Frcs(2,inw) =   ptxAB(1,inw)*cel_fy
            !Frcs(3,inw) =   ptxAB(1,inw)*cel_fz
...
!     if this is called, velforce should be switched off
            q1(i,j,k)=q1(i,j,k)+0.5*(for_xc(i,j,k)+for_xc(i-1,j,k))
            q2(i,j,k)=q2(i,j,k)+0.5*(for_yc(i,j,k)+for_yc(i,j-1,k))
            q3(i,j,k)=q3(i,j,k)+0.5*(for_zc(i,j,k)+for_zc(i,j,k-1))
      
            !q1(i,j,k)=q1(i,j,k)+Frcs(1,inw)
            !q2(i,j,k)=q2(i,j,k)+Frcs(2,inw)
            !q3(i,j,k)=q3(i,j,k)+Frcs(3,inw)
      inw = inw+1
        end do
       end do
      end do 
