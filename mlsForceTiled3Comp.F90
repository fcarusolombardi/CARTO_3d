...
!            write(*,*) 'f',d(i)
      ...
      subroutine update_forces_kernel_1cmp(cmp, siz, str1, str2, &
        ...
        n2m, for_xc, for_yc, for_zc, pindt, tri_ver, tri_vel,vel_tri, albegaBar, &
        faceid_t, tri_tiling, tstart, mytl, tlcnt, q1, q2, q3, &
        sur, xm, ym, zm, xc, yc, zc, Hboxx, celvol, nttot, nftot, rigamax, Navamax, n1, n2, n3, ks, ke, lvlhalo, &
        face_to_part,body_nsl, body_flux,tri_nor,dt,nsfac)
        ...
        real(DP), value, intent(in) :: dx1, dx2, dx3, wscl, wcon, dt,nsfac
        integer, device, dimension(1), intent(in) :: tlcnt

        integer, device, dimension(*), intent(in) :: body_nsl
        real(DP), device, dimension(*), intent(in) :: body_flux
        integer, device, dimension(nftot), intent(in) :: face_to_part
        
        integer, device, dimension(6, nttot), intent(in) :: pindt
        ...
        integer :: tid, tx, ty, tl, tile, ntr, inp
...
!        Forx = B(1) * cfac * blend * (vel_tri(cmp,ntr) - Ui(1))
        ...
        elseif (body_nsl(inp) .EQ. 0) then           
           Forx = nsfac*dt*body_flux(inp)*B(1)*cfac*tri_nor(cmp,ntr) !FV
           !        write(*,*) Forz
        else
           Forx = 0.0D0
        endif
...
!     --------------------------------------------------------
      ...
      ! JDR NOTE: Not using dum_for with current code.
#ifdef CONTACT
      !dum_for = -1
#endif
#ifdef CONTACTV
      ...
      blocks = dim3(ceiling(real(nftot)/128), 1, 1)
      call get_valid_triangles<<<blocks, threads>>>(trcnt(1), mytr, pind, &
                                                    nftot, kstart, kend)
      istat = cudaMemcpy(trcnt_h, trcnt(1), 1, cudaMemcpyDeviceToHost)
#else
      ! precompute valid triangles
      tr = 1
      trcnt = 0
...
#ifdef CONTACTV
      ! Collision detection based on vertex (new version)
      ! TODO: can merge inp and ntr loops
!VALVOLA AORTICA
!PRIMA
#ifdef USE_CUDA
        !$cuf kernel do (1)
#endif
        do ntr = vstart(2),vend(2) !aumenta vend fino a 5 (vstart fisso 2)
          ind_pal = pindv(6,ntr)
...
!PRIMA
        ...
        !$cuf kernel do (1)
#endif
        do ntr = vstart(3),vend(3) !aumenta vend fino a 5 (vstart fisso 2)
          ind_pal = pindv(6,ntr)
!          if(ind_pal .ge. kstart-1 .and. ind_pal .le. kend-1) then
          if(ind_pal .ge. kstart .and. ind_pal .le. kend-1) then
            dum_forv(ntr) = 0
...
!PRIMA
        ...
        !$cuf kernel do (1)
#endif
        do ntr = vstart(4),vend(4) !aumenta vend fino a 5 (vstart fisso 2)
          ind_pal = pindv(6,ntr)
!          if(ind_pal .ge. kstart-1 .and. ind_pal .le. kend-1) then
          if(ind_pal .ge. kstart .and. ind_pal .le. kend-1) then
            dum_forv(ntr) = 0
...
!PRIMA
        ...
        !$cuf kernel do (1)
#endif
        do ntr = vstart(5),vend(5) !aumenta vend fino a 5 (vstart fisso 2)
          ind_pal = pindv(6,ntr)
!          if(ind_pal .ge. kstart-1 .and. ind_pal .le. kend-1) then
          if(ind_pal .ge. kstart .and. ind_pal .le. kend-1) then
            dum_forv(ntr) = 0
...
!AORTA
        ...
        !$cuf kernel do (1)
#endif
        do ntr = vstart(1),vend(1) !aumenta vend fino a 5 (vstart fisso 2)
        chamb = vert_to_chamb(ntr)
        if (chamb.EQ.5) then
           ind_pal = pindv(6,ntr)
!          if(ind_pal .ge. kstart-1 .and. ind_pal .le. kend-1) then
           if(ind_pal .ge. kstart .and. ind_pal .le. kend-1) then
...
!@cuf istat = cudaDeviceSynchronize !JDR TMP
      ...
      tlcnt = 0
      threads = dim3(128, 1, 1)
      blocks = dim3(ceiling(real(nttot)/128), 1, 1)
!      ! Can reuse get_valid_triangles kernel
      call get_valid_triangles<<<blocks, threads>>>(tlcnt(1), mytl, pindt, &
                                                    nttot, kstart, kend)
#else
      tl = 1
      tlcnt = 0
      do tile = 1, nttot
        ind_pal = pindt(6,tile)
        if(ind_pal .ge. kstart .and. ind_pal .le. kend-1) then
          mytl(tl) = tile
          tl = tl + 1
          tlcnt(1) = tlcnt(1) + 1
...
!@cuf istat = cudaDeviceSynchronize !JDR TMP
      ...
      threads = dim3(32, 4, 1)
      blocks = dim3(ceiling(real(nttot)/4), 1, 1)

      do cmp = 1, 3
        call update_forces_kernel_1cmp<<<blocks, threads>>>(cmp, siz, stride(1), stride(2), &
        nel, dx1, dx2, dx3, wscl, wcon, wcub, wexp, &
        n1m, n2m, for_xc, for_yc, for_zc, pindt, tri_ver, tri_vel,vel_tri, albegaBar, &
        faceid_t, tri_tiling, tstart, mytl, tlcnt, &
        q1, q2, q3, sur, xm, ym, zm, xc, yc, zc, Hboxx, celvol, &
        nttot, nftot, size(albegaBar, 1), size(albegaBar, 2), n1, n2, n3, kstart, kend, lvlhalo, face_to_part,body_nsl, body_flux,tri_nor,dt,nsfac)
      end do
    ...
    !     For moving particles
          ...
          elseif (body_nsl(inp) .EQ. 0) then          
             Forx = nsfac*dt*body_flux(inp)*tri_nor(cmp,ntr)
          else
             Forx = 0.0D0
          endif
